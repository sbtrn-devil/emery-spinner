// this is emery-spinner runtime, will be emitted to target HTML
(function (exports) {
		...
		runtime = {
			...
			loadLinks(links) {
				for (var link of links) {
					...
					script.async = false;
					script.defer = true;
					script.src = link.file;
					script.charset = link.charset;
					document.body.appendChild(script);
				}
				// remove self from the page to save on storing the links
				runtime.asyncRemoveCurrentScript();
			}
		},
		REGEXP_RESID = /^#([-A-Za-z0-9._\(\)\[\]\{\}$%!]+)@/,
		REGEXP_STD_RESREF = /^#([-A-Za-z0-9._\(\)\[\]\{\}$%!]+)@((?:[^\/]*\/)*)([^\/:]*)(?:(?::)([\S\s]*))?$/,
		REGEXP_STD_GROUP_SLASH_TRIM = /(^\/+)|(\/+$)/g;
	var R$accessed = false;
	...
	function preprocessValue(v) {
		...
		for (var preprocessor of preprocessors) {
			...
			if (typeof (preprocessedValue) !== 'undefined') {
				v = prepV = preprocessedValue;
				break;
			}
		}
		...
		if (typeof (prepV) !== 'undefined') return prepV;
	}
	...
	function preprocessComponents(v) {
		...
		for (var k in v) {
			...
			if (typeof (prepV) !== 'undefined') {
				changes[k] = prepV;
			} else if (typeof (v[k]) === 'object') {
				preprocessComponents(v[k]);
			}
		}
	...
	runtime.asyncRemoveCurrentScript = function asyncRemoveCurrentScript() {
		var curScript = document.currentScript;
		setTimeout(function () { curScript.remove(); }, 0);
	};

	runtime.zEmit = function zEmit(srcB64z) {
		var curScript = document.currentScript,
			script = document.createElement('script');
		script.async = false;
		script.defer = true;
		script.text = runtime.pako.inflate(b64decode(srcB64z), { to: 'string' });
		document.body.appendChild(script);
		// remove self from the page to save on storing the b64z'ed source
		runtime.asyncRemoveCurrentScript();
	};

	runtime.emit = function emit(src) {
		var curScript = document.currentScript,
			script = document.createElement('script');
		script.async = false;
		script.defer = true;
	...
	runtime.registerValuePreprocessor = function registerValuePreprocessor(preprocess) {
		if (typeof(preprocess) !== 'function') {
			throw new Error("registerValuePreprocessor argument must be a function with args (value, [preprocessValue])");
		}
		if (R$accessed) {
			throw new Error("Can register no more value preprocessors - resources from R$ have already been accessed");
		}

		preprocessors.push(preprocess);
	};

	runtime.preprocessValue = preprocessValue;

	runtime.parseStdResRef = function parseStdResRef(resRef) {
		if (typeof (resRef) != 'string') return null;
		var match = resRef.match(REGEXP_STD_RESREF);
		if (!match) return null;
		return {
			type: match[1],
			path: "/" + match[2].replace(REGEXP_STD_GROUP_SLASH_TRIM, ""),
			name: match[3] || "",
			hint: match[4] || "",
			get group() { return this.path == "/" ? "" : this.path; },
			get resId() { return "#" + this.type + "@" + this.path + (this.path == "/"? "" : "/") + this.name; }
		};
	};
